...
class VariableHeader extends HeaderBase {
	...
	public static function beforeRun(&$report) {
		foreach($report->options['Variables'] as $var=>$params) {
			...
			//if the type is daterange, parse with strtotime
			if($params['type'] === 'daterange' && $report->macros[$params['name']]['start']) {

				$time = strtotime($report->macros[$params['name']]['start']);
				if(!$time) throw new Exception($params['display']." must be a valid datetime value.");

				$report->macros[$params['name']]['start'] = date($params['format'],$time);
			}
			if($params['type'] === 'daterange' && $report->macros[$params['name']]['end']) {

				$time = strtotime($report->macros[$params['name']]['end']);
				if(!$time) throw new Exception($params['display']." must be a valid datetime value.");

				$report->macros[$params['name']]['end'] = date($params['format'],$time);
			}
